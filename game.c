...
// Movement directions, match to the gamepad buttons bits to simplify some things
#define DIR_NONE		0
...
// Numbers for screens that are displayed by the same function as the level number
...
// Palettes data
// All palettes are designed in NES Screen Tool, then copy/pasted here with Palettes/Put C data to clipboard function
...
const unsigned char updateListData[] =
{
	...
	0x28,0x00,TILE_EMPTY,
	0x20,0x4f,0x10,			// These three entries are used to display
	0x20,0x50,0x10,			// number of collected items
	0x20,0x51,0x10,
	NT_UPD_EOF
};

// Nametable string with game stats, created in NES Screen Tool and copy/pasted with Shift+C
...
// List of screens, with pointers to packed nametables
...
// Large number (2x3 tiles each) nametable definitions, for showing level numbers
...
void title_screen(void)
{
	...
	scroll(-8,0); // If start is pressed, show the title in full
...
{
	scroll(-4,0); // All the screens are misaligned horizontally by half of a tile

	if (num < LEVELS_ALL) 	spr = 0;
	else 					spr = num-LEVELS_ALL+1; // Get offset in the screens list
	...
	{
		j = num<<1;
		i16 = 0x2194; // Position of number in the nametable
...
void game_loop(void)
{
	...
	while (!game_done)
	{
		...
		// Slowly fade virtual brightness to needed value, which is max for gameplay or half for pause
		if (!(frame_cnt&3))
		{
		...
		// Poll the gamepad in trigger mode
		i = pad_trigger(0);
		...
		if (wait)
		{
			...
			if (!wait) music_play(MUSIC_GAME); // Start music when all objects spawned
		}
		...
		// Player and enemies are the same type of object in this game to make code simpler and shorter,
		// but generally they need to be different kind of objects
		for (i = 0; i < player_all; ++i)
		{
			...
			if (i)
			{
				...
				{
					// If an enemy touches the player, quit the game loop
					if (!game_clear)
					{
			...
			if (player_cnt[i])
			{
				...
				if (player_cnt[i] <= 0)
				{
					...
					// If it is the player object, check if there is an item in the new tile
					if (!i)
					{
			...
			if (!player_cnt[i]) // Movement to the next tile is done, set up new movement
			{
				if (!i) // This is the player, process controls
				{
					...
					// When you press two directions at once, sliding by a wall to take turn into a passage on the side
					// This piece of code gives current direction lower priority through testing it first
					// Bits in player_dir var match the buttons bits
					if (j&player_dir[0])
					{
